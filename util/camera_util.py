...
def resize_img(img, output_pixel_size, method='linear'):
    img = jnp.array(img)
    ...
    resize_output_shape = (cropped.shape[0], *output_pixel_size, cropped.shape[-1])
    img_rsized = jax.image.resize(cropped, resize_output_shape, method=method)
    return img_rsized.astype(origin_dtype)


def visualize_pcd(rgb_list_, depth_list_, cam_posquat_, intrinsic_, return_elements=False, pcd_o3d=None):
    ...
    if pcd_o3d is None:
        pcd_o3d = o3d.geometry.PointCloud()
    pcd_o3d.points = o3d.utility.Vector3dVector(pcd_tf.reshape(-1,3))
    pcd_o3d.colors = o3d.utility.Vector3dVector(rgb_list_.reshape(-1,3)/255.)
    mesh_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(
        size=0.1, origin=[0, 0, 0])
    if not return_elements:
        o3d.visualization.draw_geometries([*cam_vis_list, *cam_pq_mesh_list, pcd_o3d, mesh_frame])
    else:
        return [*cam_vis_list, *cam_pq_mesh_list, pcd_o3d, mesh_frame]
